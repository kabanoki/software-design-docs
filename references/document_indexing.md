...
1. **Read the existing index first.** Look for `Documents/INDEX.md`, `docs/INDEX.md`, `README.md`, `AGENTS.md`, or project-specific documentation maps before creating files.
...
4. **Do not delete old documents by default.** Move obsolete but potentially useful docs to `archive/`, `archive/old/`, or the project's established historical folder.
5. **Limit AI reference scope.** The index should say which documents to read for each task and which folders are normally historical only, such as `reviews/` and `archive/`.
6. **Record source status.** Distinguish canonical docs, drafts, examples, review outputs, generated packets, and obsolete documents.
...
- `doc_type`: `requirements`, `basic_design`, `detailed_design`, `db_design`, `api_design`, `batch_design`, `screen_design`, `test_viewpoints`, `impact_analysis`, `cautions`, `runbook`, `review`, `archive`.
- `status`: `canonical`, `draft`, `example`, `review_output`, `obsolete`.
